//
// Functions for reading & writing .ply files
// that allow you to use custom fields.
//
//
// David Butterworth
//
...
// MOVE template functions to .hpp ?
...
// Write a .ply file with 3 properties:
// x, y, z, 

void writePlyFile(const std::string& filename, const std::vector<float>& xyz_values, const bool binary_format);
...
void writePlyFile(const std::string& filename,
                  const std::vector<float>& xyz_values,
...
//void add_properties_to_element(const std::string & elementKey, const std::vector<std::string> & propertyKeys, const std::vector<T> & source, const int listCount = 1, const PlyProperty::Type listType = PlyProperty::Type::INVALID)
