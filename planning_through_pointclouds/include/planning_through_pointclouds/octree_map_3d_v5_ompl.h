...
// Octree Map 3D v5
//
// Similar to v4, but this version makes the min & max height
// constraint optional

// optional elevation map

// add 2d and 3d separate
...
class OctreeMap3D
{
  public:
    ...
    const std::vector<Point> findPath3D(const Point& start_point,
...
// note: not strictly elevation
    void setElevationMap(PointCloud::Ptr& pointcloud);
    ...
    // For an x,y point, get the height of the ground terrain.
    const float getGroundHeight(const Point& point);


    // For a point, get the altitude relative to the ground.
    const float getAltitude(const Point& point);
...
// similar in idea to elevation map, but relative to sensor not sea level.
    PointCloud::Ptr ground_pointcloud_; // xyz points representing the ground terrain
    pcl::KdTreeFLANN<Point> ground_xy_kdtree_;
    ...
    const bool isStateValid_CheckSphere_CheckAltitude(const ompl::base::State* state);
