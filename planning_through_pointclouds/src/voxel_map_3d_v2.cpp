  ...
  , downsampled_pointcloud_(new PointCloud())
{
  ...
  // distances + predecessors + adjacency list
  const uint vertices_memory_mb = ((num_vertices * 2 * num_bytes_each) + (num_vertices * 6 * num_bytes_each)) / 1000000;

  // Estimate: add 6 edges for each vertice, being 3 numbers
  const uint edges_memory_mb = estimated_num_edges * 3 * num_bytes_each / 1000000;
...
void VoxelMap3D::addGraphVertices()
{
  ...
  staggered_adjacency_list_.resize(vertex_count);
            ...
            for (IndexType i = 0; i < 6; ++i)
            {
                   ...
                   && (neighbour_z >= 0) && (neighbour_z <= occupancy_grid_depth_-1))
              {
                ...
                if (occupancy_grid_3d_.at(neighbour_x).at(neighbour_y).at(neighbour_z).at(0) == FREE_SPACE)
                {
                  ...
                  staggered_adjacency_list_.at(vertex_ID).push_back(Vertex(neighbour_vertex_ID, weight));
                                                  ...
                                                  const bool use_astar)
{
  ...
  while (!pq.empty() && !found_goal)
  {
    ...
    if (staggered_adjacency_list_.at(u).size() == 0)
    {
    ...
    for (size_t i = 0; i < staggered_adjacency_list_.at(u).size(); ++i)
    {
      // Alternative distance = distance to current vertex (u) + distance to neighbour vertex (it's weight)
      const float alt_dist = distances_.at(u) + staggered_adjacency_list_.at(u).at(i).weight;

      if (alt_dist < distances_.at(staggered_adjacency_list_.at(u).at(i).vertex))
      {
        distances_.at(staggered_adjacency_list_.at(u).at(i).vertex) = alt_dist;
        predecessors_.at(staggered_adjacency_list_.at(u).at(i).vertex) = u;

        float heuristic = 0.0;
        if (use_astar)
        {
          // Cost to goal
          heuristic = euclideanDistance(staggered_adjacency_list_.at(u).at(i).vertex, goal_vertex_ID);
          // For tie-breaking.
          // This provides big speed-up too, but breaks 'admissibility'.
          heuristic *= 1.1;
        }
        // Add this neighbouring vertex to the queue to be examined
        pq.push(Vertex(staggered_adjacency_list_.at(u).at(i).vertex, alt_dist + heuristic));
      }
    }
  }
