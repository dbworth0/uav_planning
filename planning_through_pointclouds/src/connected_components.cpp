...
#include <planning_through_pointclouds/connected_components.h>



// Based on Jason Newton's implementation of Wu's algorithm that was contributed to OpenCV 3.0
...
namespace cv
{
    ...
    */
    ...
    //struct LabelingImpl{
    struct LabelingWu{
    LabelT operator()(const cv::Mat &I, cv::Mat &L, int connectivity){
...
int findConnectedComponents(const cv::Mat& I, const int connectivity, cv::Mat& L)
{
    ...
    CV_Assert(iDepth == CV_8U || iDepth == CV_8S);
        ...
        if (lDepth == CV_8U)
        {
            return (int)LabelingWu<uchar, uchar>()(I, L, connectivity);
        }


        else if (lDepth == CV_16U)
        {
            return (int)LabelingWu<ushort, uchar>()(I, L, connectivity);
        }
        else if (lDepth == CV_32S)
        {
            return (int)LabelingWu<int, uchar>()(I, L, connectivity);
        }
