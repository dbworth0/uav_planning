...
#include <planning_through_pointclouds/pcl_utils.h> // loadPointCloud, getPointCloudBounds, downsamplePointCloud
...
// TODO: This checks for collisions at the resolution of the interpolated path,
//       which could be too fine. Could check at X distance.
const bool OctreeMap3D::isPathInCollision(std::vector<Point>& path)
{
  for (size_t i = 0; i < path.size(); ++i)
  {
    if (isInCollisionSphere(path.at(i), robot_radius_))
    {
      return true;
    }
  }
