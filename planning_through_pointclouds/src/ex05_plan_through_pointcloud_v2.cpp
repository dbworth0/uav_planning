...
//
// The graph structure is a vertex adjacency list (staggered matrix)
// which uses much less memory than BGL (Boost Graph Libray).
//
...
// ex05_plan_through_pointcloud_v2 --input ./cropped_cloud_1.ply --start 67.0 2.0 -2.0 --goal 65.0 39.0 -1.0 --output ~/path_out2_26conn.ply
//
// After the occupancy grid and graph have been created, planning takes 20ms.
//
// David Butterworth
//
...
int main(int argc, char **argv)
{
  ...
  map.buildGraphFromOccupancyGrid();
  std::cout << "RAM used by graph edges: " << (getProcessPhysicalMemory() - ram_used)/1000 << " MB" << std::endl;
  ram_used = getProcessPhysicalMemory();
